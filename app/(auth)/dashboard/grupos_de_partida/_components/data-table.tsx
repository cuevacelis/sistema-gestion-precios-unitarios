...
export default function TableComponent({
	...
	const handleNavigateToSubgroup = (grupoPartidaId: number) => {
		router.push(`${pathname}/${grupoPartidaId}?${searchParams.toString()}`);
	};

	const handleRowClick = (row: IDataDBObtenerGruposDePartidasId) => {
		const rowId = row.grupar_id.toString();
		if (isMobile) {
			router.push(`${pathname}/${row.grupar_id}?${searchParams.toString()}`);
		} else {
		...
		return (
			<section className="flex flex-col items-center justify-center min-h-[400px]">
				...
				<Button asChild variant="default" className="mt-2">
					<Link
						href={`/dashboard/partidas?grupoPartidaId=${lastGrupoPartidaId?.toString()}`}
						scroll={false}
					>
						<ModuleIconsComponent
											...
											<>
												{row.original.tiene_partidas ? (
													<>
														<ContextMenuItem>
															<Link
																href={`/dashboard/partidas?grupoPartidaId=${row.original.grupar_id.toString()}`}
																scroll={false}
																className="flex items-center"
															>
																<ModuleIconsComponent
														...
														<ContextMenuItem>
															<Link
																href={`/dashboard/partidas/crear?grupoPartidaId=${row.original.grupar_id.toString()}`}
																scroll={false}
																className="flex items-center"
															>
																<ModuleIconsComponent
